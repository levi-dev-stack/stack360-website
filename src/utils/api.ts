...
import type { ApiErrorHandlerParams, ApiResponse } from '@/types/api';

export const openAsBlobInNewTab = async (certificateUrl: string) => {
  const response = await fetch(certificateUrl, { method: 'GET', credentials: 'include' });
...
export const withErrorHandler = <T, A extends unknown[]>(apiCall: (...args: A) => Promise<T>) => {
