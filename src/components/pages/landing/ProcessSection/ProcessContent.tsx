    ...
    <section className="site-section w-full border-t border-neutral-200 bg-neutral-50 py-2xl">
      <div className="site-container">
        ...
        >
          ...
          <motion.p
            variants={motionVariants(reduced, fadeUp)}
            className="max-w-content text-sm leading-relaxed text-neutral-600"
          >
        ...
        <ol className="grid grid-cols-1 gap-md sm:grid-cols-2 lg:grid-cols-4">
          {LANDING_PROCESS.map((step, index) => (
            <motion.li
              ...
              className="relative border-t-2 border-primary/40 pt-lg"
            >
              <p className="font-mono text-xs font-bold uppercase tracking-wider text-neutral-600">
                Phase {index + 1}
              </p>
              <h3 className="mt-sm text-lg font-bold tracking-tight text-neutral-900">
                {step.title}
              </h3>
              <p className="mt-sm text-sm leading-relaxed text-neutral-600">{step.description}</p>
            </motion.li>
          ))}
        </ol>
      </div>
